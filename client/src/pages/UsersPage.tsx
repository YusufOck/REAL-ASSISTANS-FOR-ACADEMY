...
// 🛰️ TİPLEME: TypeScript hatalarını mühürleyen arayüzler
...
export default function UsersPage() {
  ...
  // 🔒 KİLİT VE İPTAL MEKANİZMALARI
  const abortControllerRef = useRef<AbortController | null>(null);
  const metaFetched = useRef(false); // Meta verileri (bölüm/skill) sadece 1 kez çekmek için

  // 1. ADIM: Meta Verileri Çek (Sadece 1 Kez)
  useEffect(() => {
  ...
  // 2. ADIM: Araştırmacı Listesini Çek (İptal Destekli)
  const fetchUsers = async () => {
    // 🛡️ Önceki (yavaş) isteği anında iptal et, sunucuyu yorma
    ...
    try {
      ...
      // 🛰️ İptal sinyaliyle birlikte tek istek fırlat
  ...
  // 3. ADIM: Filtre Değişimlerini İzle (Debounce: 500ms)
  useEffect(() => {
    ...
    <div className="min-h-screen bg-[#0b1020] text-slate-100 p-8 space-y-10 font-sans">
      ...
      <div className="max-w-7xl mx-auto flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
        ...
        <div className="flex flex-wrap gap-4 w-full xl:w-auto">
          ...
          {/* Yetenek Multi-Select */}
          <div className="relative">
            <Button 
              onClick={() => setShowSkillDropdown(!showSkillDropdown)}
              className="bg-white/5 border border-white/10 rounded-2xl py-7 px-6 hover:bg-white/10 flex gap-2 h-auto"
            >
                ...
                <div className="max-h-60 overflow-y-auto custom-scrollbar space-y-1">
                  {allSkills.length > 0 ? allSkills.map((s) => (
                    <div 
                      ...
                      className={`flex items-center justify-between p-3 rounded-xl cursor-pointer transition ${selectedSkills.includes(s.skill_id) ? 'bg-indigo-500/20 text-white' : 'hover:bg-white/5 text-slate-400'}`}
                    >
